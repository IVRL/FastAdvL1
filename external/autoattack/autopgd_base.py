...
class APGDAttack():
    ...
    def attack_single_run(self, x, y, x_init=None):
        ...
        if self.norm == 'Linf':
            ...
            # this initialization works worse than L1 uniform (Laplace) initialization
        
        
        
        
        if x_init is not None:
            x_adv = x.clone() if x_init == 'no_init' else x_init.clone()
        ...
        if self.norm == 'L1':
            ...
            if x_init in [None, 'no_init']:
                # topk = .2 * torch.ones([x.shape[0]], device=self.device)
                topk = self.p * torch.ones([x.shape[0]], device=self.device)
